...
public class LoginPage extends Page{
	...
	public void loginCheck(){
		...
		if(storeMemberDTO==null) {
			...
			//로그인체크가 완료되면 foodItemd의 삭제버튼은 비활성화 되어야한다 근데 어떻게 하지?
			System.out.println("접속아이디는 : "+storeMemberDTO.getId());
			registPage=(RegistPage)storeMain.pages[2];
			
			setButtonActive();
			
			registPage.showId(storeMemberDTO.getId());
			
			JOptionPane.showMessageDialog(this, "로그인 성공");
			
			storeMain.showHide(StoreMain.REGISTPAGE);
			
			//로그인 완료후 로그인 JTextField비워주기
			cleanLogin();
		}
	}
	...
	//로그인할때나 등록버튼을 누를때나 삭제할때 등 버튼을 계속해서 활성비활성화로 해주어야 하므로 하나의 메서드로
	//빼주어서 관리하자!
	public void setButtonActive() {
		//이 부분은 중복가능성이 다분하고 역할이 다르므로 메서드로 따로 빼두자!
		for(int i=0; i<registPage.foodList.size(); i++) {
			FoodItem foodItem=registPage.foodList.get(i);
			foodItem.closeButton(storeMemberDTO.getId());
		}
	}
