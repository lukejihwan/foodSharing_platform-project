...
//	StoreMenuDTO storeMenuDTO;
	...
	public int insert(StoreMenuDTO storeMenuDTO){ //이거는 멤버에 올릴필요없이 {}안에서 다 사용하면 될듯
		...
		System.out.println("등록할 storemember_idx값은 : "+storeMenuDTO.getStoreMemberDTO().getStoreMember_idx());
	...
	public List<StoreMenuDTO> selectAll() {
		...
		String sql="SELECT id, STORENAME, ADDRESS, FOODCATEGORY, STOREMENU_IDX, m.STOREMEMBER_IDX, FOODNAME, SERVINGS, DEADLINE, MEMO, FILENAME";
		sql+=" FROM storemember s right OUTER JOIN storemenu m";
		sql+=" on s.STOREMEMBER_IDX =m.STOREMEMBER_IDX ORDER BY STOREMENU_IDX desc";
	...
	public int delete(int storeMenu_idx) {
		int result=0;
		Connection con=null;
		PreparedStatement pstmt=null;
		con=dbManager.getConnection();
		
		String sql="delete from storemenu where storemenu_idx=?";
		try {
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, storeMenu_idx);
			pstmt.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
		}finally {
